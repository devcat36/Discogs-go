...
const MockList_WA = (count) => {
  const limit = Array.isArray(count) ? Math.floor(Math.random() * (count[1] - count[0] + 1) + count[0]) : count;
...
export const mocks = {
  ...
  Filter: () => {
    ...
    if (category === 'CURRENCY')
      ...
      ret.name = casual.random_element(['VINYL', 'ACETATE', 'FLEXIDISC', 'CD', 'DVD', 'BLURAY', 'SACD']);
    return ret;
  },
  Item: () => ({
