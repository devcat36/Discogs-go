...
import { CONDITION_OPTIONS, CURRENCY_SYMBOL, FORMAT_OPTIONS, GENRE_OPTIONS } from '../constants';
...
function Marketplace() {
  ...
  let selectedFilters = [
    ...
    queryParam.price && {
      category: 'Price',
      value: filters.price,
      text: `${CURRENCY_SYMBOL[filters.currency]}${filters.price.split('to')[0]} to ${
        CURRENCY_SYMBOL[filters.currency]
      }${filters.price.split('to')[1]}`,
    },
    queryParam.genre &&
    ...
    queryParam.media_condition &&
      filters.mediaCondition.map((mediaCondition) => ({
        ...
        value: CONDITION_OPTIONS.find((option) => option.value === mediaCondition).value,
        text: CONDITION_OPTIONS.find((option) => option.value === mediaCondition).text,
      })),
    queryParam.sleeve_condition &&
      filters.sleeveCondition.map((sleeveCondition) => ({
        category: 'Sleeve Condition',
        value: CONDITION_OPTIONS.find((option) => option.value === sleeveCondition).value,
        text: CONDITION_OPTIONS.find((option) => option.value === sleeveCondition).text,
      })),
    queryParam.year &&
    ...
    <Grid columns={2}>
      <Grid.Row>
        ...
        <Grid.Column width={13}>
          {data.search.result.filter((result) => result.__typename === 'Item').length > 0 ? (
            <ItemList
              items={data.search.result.filter((result) => result.__typename === 'Item')}
